...
export async function getJob(db: SQLiteDatabase, id: string): Promise<Job | null> {
...
export async function createJob(db: SQLiteDatabase, input: JobInput): Promise<Job> {
  const job: Job = {
    id: newId(),
...
export async function updateJob(
  ...
  await db.runAsync(`UPDATE jobs SET ${sets.join(', ')} WHERE id = ?`, ...values, id);
}

export async function archiveJob(db: SQLiteDatabase, id: string): Promise<void> {
  await db.runAsync('UPDATE jobs SET archived_at = ? WHERE id = ?', Date.now(), id);
}

export async function unarchiveJob(db: SQLiteDatabase, id: string): Promise<void> {
