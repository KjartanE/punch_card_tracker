...
export async function getSettings(db: SQLiteDatabase): Promise<Settings> {
  ...
  if (!row) throw new Error('Settings row missing — migration did not seed row.');
  return toSettings(row);
}

export type SettingsPatch = Partial<Settings>;

export async function updateSettings(db: SQLiteDatabase, patch: SettingsPatch): Promise<void> {
  const sets: string[] = [];
  ...
  await db.runAsync(`UPDATE settings SET ${sets.join(', ')} WHERE id = 1`, ...values);
}
