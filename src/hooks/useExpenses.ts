...
export function useExpenses(opts: ListExpensesOpts = {}): ExpenseView[] | null {
  ...
  const { clientId, jobId, category, occurredAtFrom, occurredAtTo, limit } = opts;
...
export function useExpense(id: string | null | undefined): ExpenseView | null | undefined {
  const db = useSQLiteContext();
  const version = useInvalidation((s) => s.versions.expenses);
  const [expense, setExpense] = useState<ExpenseView | null | undefined>(undefined);
